...
function* updateNameSaga(action) {
    try {
        ...
        yield axios.patch(`http://localhost:4000/phoneNumbers/${id}`, {
            phoneNumber: values.phoneNumber,
        });

        yield axios.patch(`http://localhost:4000/emails/${id}`, {
            email: values.email,
        });

        yield axios.patch(`http://localhost:4000/images/${id}`, {
            url: values.images[0].url,
            name: values.images[0].name,
            type: values.images[0].type,
            thumbUrl: values.images[0].thumbUrl,
        });
...
export default function* userSaga() {
    ...
    yield takeEvery(REQUEST(USER_ACTION.UPDATE_CUSTOMER), updateNameSaga);
}
